...
function tryParseStartOrSelfClose(input: string, pos: number): { el: HVElement; end: number } | null {
  ...
  if (input.startsWith("/>", i) || input.startsWith("@/>", i)) {
    const end = i + (input.startsWith("@/>", i) ? 3 : 2);
    return { el: { type: "selfclose", name, args }, end };
  }

  if (input[i] === ">") {
    return { el: { type: "start", name, args }, end: i + 1 };
  }
...
export function parse(input: string): HVElement[] {
  ...
  while (i < input.length) {
    ...
    if (input.startsWith("</@", i)) {
      const result = tryParseEndTag(input, i);
      if (result) { elements.push(result.el); i = result.end; continue; }
    }

    if (input.startsWith("<@", i)) {
      const result = tryParseStartOrSelfClose(input, i);
      if (result) { elements.push(result.el); i = result.end; continue; }
    }
